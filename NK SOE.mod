...
@#else
    ...
    @#else
        ...
        @#else
            @#if NGDP ==1
                case_title='nominal gdp targeting (NGDP)';
                short_title='NGDP';
            @#else
                @#if FCIT ==1
...
var y_gap       ${\tilde{y}}$   (long_name='output gap')los
...
parameters betta    ${\beta}$       (long_name='discount factor')
    ...
    phi_g           ${\phi_{g}}$    (long_name='ngdp feedback rule') 
    ;
...
pi_h  = betta*pi_h(+1) + kappa_upsilon*y_gap + u;
...
%----------------------------------------------------------------
stoch_simul(order = 1,irf=15) y_gap n pi_h_ann g pi_ann p_h p s er i_ann r_real_ann;
